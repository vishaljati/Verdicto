...
const listPersonas = AsyncHandler(
    async (req, res) => {
        try {
...
const getPersona = AsyncHandler(async (req, res) => {
    try {
        ...
        if (!persona) {
            return res.status(404).json(new ApiResponse(404, {}, "Personas not found"));
        }

        return res.status(200).json(new ApiResponse(404, { persona }, "Personas fetched Successfully"));
    } catch (err) {
        console.error("Error fetching persona:", err);
        return res.status(500).json({ success: false, message: "Failed to fetch persona" });
    }
})
...
const createPersona = AsyncHandler(async (req, res) => {
    try {
        ...
        return res.status(201).json(new ApiResponse(201, { persona }, "Persona created successfully"));
    } catch (err) {
        console.error("Error creating persona:", err);
        return res.status(500).json({ success: false, message: "Failed to create persona" });
    }
}
)
...
const updatePersona = AsyncHandler(async (req, res) => {
    try {
        ...
        if (persona.user && persona.user.toString() !== userId.toString()) {
            return res.status(403).json(new ApiResponse(403, {}, "Not allowed to update this persona"));
        }
        ...
        return res.status(200).json(
            new ApiResponse(200, { persona }, "Persona Updated Successfully")
        );
    } catch (err) {
    ...
    try {
        ...
        if (persona.user === null) {
            return res.status(403).json(new ApiResponse(
                403,{},"Default personas cannot be deleted"
            ));
        }

        if (persona.user.toString() !== userId.toString()) {
            return res.status(403).json(
                new ApiResponse(403,{},"Not allowed to delete this persona")
            );
        }

        await Persona.findByIdAndDelete(personaId);

       return res.status(200).json(new ApiResponse(200,{},"Persona deleted successfully"));
    } catch (err) {
        console.error("Error deleting persona:", err);
        return res.status(500).json({ success: false, message: "Failed to delete persona" });
    }
})
